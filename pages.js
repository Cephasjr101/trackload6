    ...
    async function render(){
      try {
        if (truckList) {
          ...
          truckList.innerHTML = (cachedTrucks && cachedTrucks.length) ? cachedTrucks.map(truckCard).join("") : '<div class="empty-state">No trucks listed yet — be the first.</div>';
        }
        if (loadList) {
          loadList.innerHTML = '<div class="empty-state">Loading loads…</div>';
          cachedLoads = await API.listLoads();
          loadList.innerHTML = (cachedLoads && cachedLoads.length) ? cachedLoads.map(loadCard).join("") : '<div class="empty-state">No loads posted yet — be the first.</div>';
        }
      } catch (err) {
